      ...
      | (
          ...
          | (
              [
                ...
                | "B\(.band)\(if (.bandwidth // "") != "" then "@\(.bandwidth) MHz" else "" end)"
              ]
              | if length > 0 then
                  join(" + ")
                elif $band != "" then
...
perform_dd() {
  ...
  if echo "$output" | grep -iq "error\|can't"; then
    print_message "Error overwriting partition. Please use loader" "$RED"
    umountFS
    exit_function
  fi
}
...
checking_mtd_size() {
  ...
  if echo "$output_file" | grep -qE '^/dev/mtdblock[0-9]+'; then
    ...
    if [ "$file_size" -gt "$part_size" ]; then
      print_message "The file is larger than the selected partition. Please use the uploader" "$RED"
...
select_drive() {
  ...
  if ! ensure_jq; then
    print_message "Cannot install demo. jq" "$RED"
    return 1
  fi
  ...
  if ! parsed_output=$(printf '%s\n' "$media_output" | jq -r '
    ...
    | @tsv
  ' 2>/dev/null); then
    print_message "Failed to process drive list" "$RED"
    return 1
  fi
...
exit_function() {
  ...
  if ! read -n 1 -s -r -p "Press any key to return..."; then
    echo ""
    exit 0
  fi
  pkill -P $$ 2>/dev/null
...
service() {
  ...
  if [ -n "$mtdFile" ]; then
    ...
    print_message "Failed to prepare new service data" "$RED"
    exit_function
  fi
